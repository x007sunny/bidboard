...
export default async function CategoriesPage() {
  ...
  return (
    <main>
      ...
      <h1 className="mt-4 text-3xl font-bold tracking-tight">Categories</h1>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {mostActive.map((g) => (
              <Link
                ...
                className="rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3 hover:border-neutral-300 dark:border-neutral-700 dark:bg-neutral-900"
              >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {groups.map((g) => (
          <Link
            key={g.name}
            href={`/?category=${encodeURIComponent(g.name)}`}
            className="rounded-2xl border border-neutral-200 bg-white p-4 hover:border-neutral-300 dark:border-neutral-700 dark:bg-neutral-900"
          >
              ...
              {g.items.slice(0, 3).map((item, i) => {
                ...
                return (
                  <li key={item.id} className="flex items-center gap-2 text-sm">
                    ...
                    {icon ? (
                      <img src={icon} alt="" className="h-5 w-5 rounded-md object-contain bg-white" />
                    ) : (
                      <span className="flex h-5 w-5 items-center justify-center rounded-md bg-neutral-100 text-[10px] font-semibold">
                        {item.title.charAt(0)}
                      </span>
                    )}
                    <span className="min-w-0 flex-1 truncate">{item.title}</span>
                    <span className="shrink-0 font-semibold text-indigo-600">{formatAUD(item.bidCents)}</span>
                  </li>
                );
              })}
            </ul>
